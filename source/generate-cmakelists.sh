...
# Additional code
{
  ...
  echo "    add_custom_command(" >> "$output_file"
  echo "        TARGET $first_target POST_BUILD" >> "$output_file"
  echo "        COMMAND \${CMAKE_COMMAND} -E remove -f \${CMAKE_SOURCE_DIR}/include/*.gch \${CMAKE_SOURCE_DIR}/include/*.pch" >> "$output_file"
  echo "        COMMENT \"Removing .gch and .pch files\"" >> "$output_file"
  echo "    )" >> "$output_file"
  echo "endif ()" >> "$output_file"
  echo "" >> "$output_file"
